    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12 bg-slate-50 min-h-screen">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div v-if="showCreateForm" class="bg-white overflow-hidden shadow-xl sm:rounded-xl border border-slate-200/80 transition-all duration-300 ease-in-out transform scale-100">
                    <div class="p-6 sm:p-8">
                        ...
                        <form @submit.prevent="submit" class="space-y-6">
                            <div>
                                <label for="title" class="block text-sm font-medium text-slate-700">Subject / Title</label>
                                <input
                                    id="title"
                                    type="text"
                                    v-model="form.title"
                                ...
                                />
                                <div v-if="form.errors.title" class="text-rose-600 text-xs mt-1">{{ form.errors.title }}</div>
                            </div>

                            <div>
                                <label for="description" class="block text-sm font-medium text-slate-700">Detailed Description</label>
                                <textarea
                                    id="description"
                                    ...
                                    class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition duration-150"
                                ></textarea>
                                <div v-if="form.errors.description" class="text-rose-600 text-xs mt-1">{{ form.errors.description }}</div>
                            </div>

                            <div class="w-full sm:w-1/3">
                                <label for="priority" class="block text-sm font-medium text-slate-700 font-semibold mb-2">Priority Level</label>
                                <div class="grid grid-cols-3 gap-2">
                                    <label
                                        v-for="prio in ['low', 'normal', 'high']"
                                        :key="prio"
                                ...
                                <div v-if="form.errors.priority" class="text-rose-600 text-xs mt-1">{{ form.errors.priority }}</div>
                            </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200/80">
                    <div class="p-6">
                        <div v-if="tickets.length === 0" class="text-center py-16">
                            ...
                            <h3 class="mt-4 text-lg font-medium text-slate-900">No support tickets yet</h3>
                            <p class="mt-1 text-sm text-slate-500">Need help? Submit your first support ticket using the button above.</p>
                        </div>
                            ...
                            <table class="min-w-full divide-y divide-slate-200">
                                <thead>
                                    <tr class="bg-slate-50/75">
                                        <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider rounded-l-lg">ID / Title</th>
                                    ...
                                    >
                                        <td class="px-6 py-5 whitespace-nowrap">
                                            <div class="flex flex-col">
                                                ...
                                                <span class="text-xs text-slate-400 mt-1 max-w-md truncate">{{ ticket.description }}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-6 py-5 whitespace-nowrap text-sm text-slate-600">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span>{{ ticket.assigned_agent ? ticket.assigned_agent.name : 'Unassigned' }}</span>
                                            </div>
                                        </td>
